...
const fetchHistory = async () => {
  ...
  try {
    ...
    const errMsg = error_ instanceof Error ? error_.message : "历史数据加载失败，请重试";
    console.error("Failed to fetch price history:", error_);
    error.value = errMsg;
  } finally {
    loading.value = false;
  }
...
<template>
  <div class="price-trend-chart-container">
    ...
    <div class="relative">
      ...
      <div
        v-if="loading"
        class="absolute inset-0 flex items-center justify-center bg-white/50 dark:bg-zinc-900/50 backdrop-blur-[1px] z-10 rounded-xl"
      >
      ...
      <div
        v-if="!loading && error"
        class="absolute inset-0 flex flex-col items-center justify-center bg-red-50/50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/20"
      >
      ...
      <!-- No Data -->
      <div
        v-if="!loading && !error && (!stats.history || stats.history.length === 0)"
        class="absolute inset-0 flex flex-col items-center justify-center bg-gray-50/50 dark:bg-zinc-800/50 rounded-xl"
      >
