...
<template>
  <div
    class="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 p-6 shadow-sm overflow-hidden relative"
  >
    ...
    <div
      class="rounded-xl p-6 flex flex-col items-center justify-center text-center space-y-3"
      :class="[currentSuggestion?.bg]"
    >
      <div class="text-4xl font-black" :class="[currentSuggestion?.color]">
    ...
    <div class="mt-6 grid grid-cols-2 gap-x-4 gap-y-3">
      <div
        v-for="(score, label) in {
          价格竞争力: decision?.price_score,
          历史低价: decision?.history_score,
          优惠力度: decision?.coupon_score,
          商家及评价分析: decision?.risk_score,
        }"
        :key="label"
        class="space-y-1"
      >
        ...
        <ElProgress
          ...
          :show-text="false"
          :stroke-width="4"
          :color="score && score > 70 ? '#10b981' : score && score > 40 ? '#f59e0b' : '#ef4444'"
        />
      </div>
    </div>

    <div
      v-if="decision?.best_platform"
      class="mt-6 pt-4 border-t dark:border-zinc-800 flex items-center gap-2"
    >
      <span class="iconify lucide--award text-yellow-500 w-4 h-4"></span>
      <div class="text-xs">
        <span class="text-gray-500 dark:text-zinc-500">全网最佳:</span>
        <span class="ml-1 font-bold text-gray-800 dark:text-zinc-200">{{
          decision.best_platform
        }}</span>
      </div>
    </div>
    ...
    <div class="mt-6 space-y-2">
      ...
      <ElProgress
        :percentage="(decision?.confidence || 0.85) * 100"
        :show-text="false"
        :stroke-width="8"
      />
    </div>
