...
<template>
  <div class="p-5 border rounded-2xl transition-all" :class="[config.bg, config.border]">
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-2">
        <span class="w-2 h-2 rounded-full" :class="[config.dot]"></span>
        <span class="font-bold text-sm" :class="[config.text]">{{ config.label }}</span>
      </div>
      <span
        class="text-[10px] text-gray-400 dark:text-zinc-500 font-medium uppercase tracking-wider"
        >Market Intelligence</span>
    </div>

    <div v-if="risks && risks.length > 0" class="space-y-3">
