...
<template>
  <div class="p-2 min-h-[300px] flex flex-col justify-center">
    ...
    <div v-else class="space-y-6 transition-all duration-700 ease-in-out opacity-100 scale-100">
      ...
      <div
        class="flex h-4 w-full rounded-full overflow-hidden shadow-inner bg-gray-100 dark:bg-zinc-800"
      >
        <div
          v-for="item in data"
          :key="item.name"
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        >
          <div class="flex items-center space-x-3">
            <div
              class="w-3 h-3 rounded-sm group-hover:scale-125 transition-transform shadow-sm"
              :class="[item.color]"
            ></div>
          ...
          <div class="flex items-center space-x-4">
            <div
              class="w-24 h-1.5 bg-gray-100 dark:bg-zinc-800 rounded-full overflow-hidden hidden sm:block"
            >
              <div class="h-full" :class="[item.color]" :style="{ width: `${item.value}%` }"></div>
            </div>
