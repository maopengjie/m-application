...
import { TicketPercent, Tag, ArrowRight } from 'lucide-vue-next';
...
<template>
  <Page title="优惠计算中心" description="自动抓取全网最新优惠券，为您计算最佳到手价。">
    ...
    <div v-else class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      <Card v-for="coupon in coupons" :key="coupon.id" class="overflow-hidden border-none transition-all hover:scale-[1.02] hover:shadow-xl">
        <div :class="['h-2', getPlatformColor(coupon.type)]"></div>
        <CardHeader class="pb-2">
          <div class="flex items-center justify-between">
            <Tag class="h-4 w-4 text-muted-foreground" />
            <span class="text-xs font-semibold uppercase text-muted-foreground">{{ coupon.type }}</span>
          </div>
        ...
        <CardContent>
          ...
          <button class="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-2 text-sm font-medium text-primary-foreground shadow-lg transition-all hover:opacity-90">
            立即领取
