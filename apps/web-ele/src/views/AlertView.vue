...
const fetchAlerts = async () => {
  ...
  try {
    ...
    const errMsg = err instanceof Error ? err.message : '获取列表失败，请检查网络后重试';
      ...
      <div v-if="alerts.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="alert in alerts" :key="alert.id" class="relative group bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 p-4 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div class="flex gap-4">
            <img :src="alert.sku?.product?.main_image" class="w-20 h-20 object-cover rounded border flex-shrink-0" @error="handleImageError" />
          ...
          <div v-if="alert.is_triggered" class="mt-3 p-2 bg-green-50 dark:bg-green-950/20 rounded-lg border border-green-100 dark:border-green-900/30">
            <div class="flex justify-between items-center text-[10px] text-green-700 dark:text-green-400 font-medium">
